''' Full SSH Example https://qxf2.com/blog/ssh-using-python-paramiko/ '''
...
class Ssh_Util:
        ...
        try:
            if self.connect():
                for command in commands:
                    ...
                    if self.ssh_error:
                        print("Problem occurred while running command: {} The error is {}".format(command,self.ssh_error))
                        result_flag = False
                    else:
                        print("Command {} execution completed successfully ".format(command))
                    self.client.close()
            else:
            ...
            print("Failed to execute the command! {}".format(command))
            self.client.close()
            result_flag = False
        return result_flag
    ...
    def upload_file(self, uploadlocalfilepath, uploadremotefilepath):
        ...
        try:
            ...
            print ("\nUnable to upload the file to the remote server {}".format(uploadremotefilepath))
            print("PYTHON SAYS: {}".format(e))
            result_flag = False
            ftp_client.close()
            self.client.close()
    ...
    def download_file(self, downloadremotefilepath, downloadlocalfilepath):
        ...
        try:
            ...
            print("\nUnable to download the file from the remote server {}".format(downloadremotefilepath))
            print("PYTHON SAYS: {}".format(e))
            result_flag = False
            ftp_client.close()
            self.client.close()
...
# ---USAGE EXAMPLES
def main():
    print("Start of {}".format( __file__))
    ...
    if ssh_obj.upload_file(ssh_obj.uploadlocalfilepath,ssh_obj.uploadremotefilepath) is True:
        print("File uploaded successfully {}".format(ssh_obj.uploadremotefilepath))
    else:
    ...
    if ssh_obj.download_file(ssh_obj.downloadremotefilepath,ssh_obj.downloadlocalfilepath) is True:
        print("File downloaded successfully {}".format(ssh_obj.downloadlocalfilepath))
    else:
